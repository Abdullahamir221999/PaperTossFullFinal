...
public class PaperThrow3D : MonoBehaviour
{
    ...
    void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.CompareTag("Floor") && !hitBin)
        {
            PlayMissSound();
    ...
    void PlayHitSound()
    {
        if (hitBinSounds == null || hitBinSounds.Length == 0)
        {
            Debug.LogWarning("hitBinSounds array is empty or not assigned.");
            return;
        }

        int soundIndex = (comboCount - 1) % hitBinSounds.Length;

        // Ensure the soundIndex is non-negative
        if (soundIndex < 0)
        {
            soundIndex = 0;
        }
