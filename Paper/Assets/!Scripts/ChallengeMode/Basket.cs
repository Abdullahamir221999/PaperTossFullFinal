...
{
    public Transform[] basketPositions;  // Assign the positions in the Inspector
    public float baseMoveDuration = 2.0f;    // Base duration for the basket to move
    public float moveSpeed = 1.0f;           // Speed multiplier for movement (higher = faster)
    ...
    private IEnumerator MoveBasket()
    {
        ...
        float adjustedMoveDuration = baseMoveDuration / moveSpeed; // Faster speed = shorter duration

        // Get the MeshFilter component (if needed)
        MeshFilter meshFilter = GetComponent<MeshFilter>();

        while (elapsedTime < adjustedMoveDuration)
        {
            // Move both the basket's transform and the mesh
            transform.position = Vector3.Lerp(startPosition, targetPosition, elapsedTime / adjustedMoveDuration);
            if (meshFilter != null)
            {
                // Update the mesh's position too
                meshFilter.transform.position = transform.position;
